...
public class ProcessingResult<V, V2> {
     ...
     * for automatic DLQ redirection on failed records.</p>
     *
     * @param message The resulting successful Record from the processor that needs to be wrapped
     ...
     * <p>
     * If you are using this in a Processor, refer to
     * {@link ProcessingResult#wrapRecordFailure(Exception, Record, String)} for easier syntax.
     * </p>
     *
     * @param e                 The exception
     * @param failedRecordValue The failed Kafka record
     ...
     * TopologyErrorHandler#catchErrors(KStream) for automatic DLQ redirection on failed records.
     * </p>
     *
     * @param e            The initial exception
     * @param failedRecord The resulting failed Record from the processor that needs
     *                     to be wrapped in a {@link ProcessingResult}
     * @param <K>          The type of the record key
     * @param <V>          The type of the ProcessingResult successful value
     * @param <V2>         The type of the ProcessingResult error value
     * @return The initial record with {@link ProcessingError} and {@link ProcessingResult}
     */
    public static <K, V, V2> Record<K,
        ProcessingResult<V, V2>> wrapRecordFailure(Exception e, Record<K, V2> failedRecord) {
     ...
     *
     * @param e              The initial exception
     * @param failedRecord   The resulting failed Record from the processor that needs to be
     *                       wrapped in a {@link ProcessingResult}
     * @param contextMessage The custom context message that will be added in the stack trace
     * @param <K>            The type of the record key
     * @param <V>            The type of the ProcessingResult successful value
     * @param <V2>           The type of the ProcessingResult error value
    ...
    /**
     * <p>Wraps a key, value and timestamp in a record with
     * {@link ProcessingResult#fail(Exception, Object, String)} as value.</p>
     * <p>The resulting stream needs to be handled with
     * TopologyErrorHandler#catchErrors(KStream) for automatic DLQ redirection
     * on failed records.</p>
     *
     * @param e           The initial exception
     * @param key         The key to put in the resulting record
     * @param failedValue The resulting failed value from the processor that needs
     *                    to be wrapped in a {@link ProcessingResult}
     * @param timestamp   The timestamp to apply on the resulting record
     * @param <K>         The type of the record key
     * @param <V>         The type of the ProcessingResult successful value
     ...
     * {@link ProcessingResult#fail(Exception, Object, String)} as value.
     * </p>
     * <p>
     * The resulting stream needs to be handled with TopologyErrorHandler#catchErrors(KStream)
     * for automatic DLQ redirection on failed records.
     ...
     * @param e              The initial exception
     * @param key            The key to put in the resulting record
     * @param failedValue    The resulting failed value from the processor that needs to
     *                       be wrapped in a {@link ProcessingResult}
     * @param timestamp      The timestamp to apply on the resulting record
     * @param contextMessage The custom context message that will be added in the stack trace
     * @param <K>            The type of the record key
     * @param <V>            The type of the ProcessingResult successful value
     ...
     */
    public static <K, V, V2> Record<K,
        ProcessingResult<V, V2>> wrapRecordFailure(Exception e, K key, V2 failedValue,
                                                   long timestamp,
                                                   String contextMessage) {
